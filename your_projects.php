<?php
    ...
    $sql = "SELECT projects.*, COUNT(projects.id_project) AS DONORS, SUM(donations.donation) AS SUM FROM `projects` INNER JOIN users ON projects.organizer_id = users.id_user LEFT JOIN `donations` ON projects.id_project = donations.id_project WHERE projects.organizer_id = ".$_SESSION['user_id']. " AND `users`.active = 1 GROUP BY projects.id_project ORDER BY start_date ASC";
